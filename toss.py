...
for i in range(7, n):
    try:
        searchandclick_byrate(f'toss_{i}',3,0.95)
        time.sleep(waitingtime)
        if i==2:    device.input_swipe(0,1000,0,0,1000)
        if i==4:    
        ...
        if i==6 or i==9:
            ...
            time.sleep(waitingtime)
            device.input_keyevent('KEYCODE_BACK')
            device.input_swipe(0,500,0,1500,1000)
            time.sleep(1)
            device.input_swipe(0,500,0,1500,1000)
    except:
        print(f'{i}번쨰 사진 클릭에서 오류가 남')
        break
# 2뒤에스와이프
# 4뒤에 back, 스와이프 많이   
# 6뒤에 back 4번 천천히
